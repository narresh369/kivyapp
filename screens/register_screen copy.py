...
class RegisterScreen(Screen):
    ...
    def save_data(self):
        Clock.schedule_once(
            lambda dt: self.show_loading("Registering user..."), 0)
        ...
        if not phone_pattern.match(family_phone):
            Clock.schedule_once(lambda dt: (
                self.hide_loading(),
                self.show_popup(
                    "Invalid Family Number", "Enter a valid 10-digit family phone number starting with 6-9.")
            ), 0)
            return
